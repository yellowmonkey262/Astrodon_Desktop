                ...
                using (StreamReader objReader = new StreamReader(fileName))
                {
                    ...
                    while (!objReader.EndOfStream)
                    {
                        ...
                        switch (whichLine)
                        {
                            ...
                            default:
                                String trnDate = Methods.cleanDate(lineContents[0]);
                                String description = Methods.cleanDescription(lineContents[1]);
            ...
            using (StreamReader objReader = new StreamReader(fileName))
            {
                ...
                while (!objReader.EndOfStream)
                {
                    ...
                    try
                    {
                        if (whichLine >= 7)
                        {
                            String tranDate = Methods.cleanDate(lineContents[0]);
                            String[] dateSplitter = { "/" };
