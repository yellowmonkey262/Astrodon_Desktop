...
namespace Astrodon {
    ...
    public partial class usrSummaryReport : UserControl {
        ...
        private void usrSummaryReport_Load(object sender, EventArgs e) {
            ...
            //MessageBox.Show(buildings.Count.ToString() + " buildings");
            repBuildings = Utilities.GetReportBuildings();
            //MessageBox.Show(repBuildings.Count.ToString() + " report buildings");
            List<Building2> rpb = repBuildings.Values.ToList();
            //dataGridView1.DataSource = rpb;
        ...
        private void CalcTotals() {
            foreach (KeyValuePair<String, Building2> mBuilding in repBuildings) {
                try {
                    ...
                    summary.Bank = building.Bank;
                    summary.Code = building.Code;
                    summary.TrustCode = building.Acc;
                    summary.BuildingName = building.BuildingName;
                    summary.BuildBal = double.Parse(buildBal.ToString("##0.00"));
                    summary.CentrecBal = double.Parse(centrecBal.ToString("##0.00"));
