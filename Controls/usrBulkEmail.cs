...
namespace Astrodon {
    ...
    public partial class usrBulkEmail : UserControl {
        ...
        private void LoadCategories() {
            ...
            c1.categoryID = String.Empty;
            c1.categoryName = "All customers";
            categoryList.Add(c1);
            foreach (KeyValuePair<String, String> cat in categoryDictionary) {
                Category c = new Category();
                c.categoryID = cat.Key;
                c.categoryName = cat.Value;
                categoryList.Add(c);
            }
                ...
                if (c.statPrintorEmail != 4 && (c.category == category || String.IsNullOrEmpty(category))) {
                    ...
                    foreach (String email in c.Email) {
                        if (!email.Contains("imp.ad-one")) {
                            el.EmailAddress += email.Replace("\"", "").Replace("\'", "").Replace(";", "").Replace(",", "") + ";";
                        }
                    }
        ...
        private void chkIncludeAll_CheckedChanged(object sender, EventArgs e) {
            if (dgCustomers.Rows.Count > 0) {
                foreach (DataGridViewRow dvr in dgCustomers.Rows) {
                    dvr.Cells[3].Value = chkIncludeAll.Checked;
                }
            }
        }
        ...
        private void btnSend_Click(object sender, EventArgs e) {
            ...
            if (!success) {
                MessageBox.Show("Error sending mail: " + status);
            } else {
                MessageBox.Show("All mails queued");
            }
        ...
        private void btnSendNow_Click(object sender, EventArgs e) {
            ...
            if (!success) {
                ...
                SendMail(msgID);
                MessageBox.Show("All mails sent");
            }
            this.Cursor = Cursors.Default;
        }
        ...
        private void cmbCategory_SelectedIndexChanged(object sender, EventArgs e) {
            if (cmbCategory.SelectedItem != null) {
                LoadCustomers(cmbCategory.SelectedValue.ToString());
            }
        }
